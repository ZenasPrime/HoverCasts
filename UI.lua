...
--
-- Purpose:
--   Builds and renders the on-hover tooltip:
--     • Header line: "<Name> (Level X Race Spec Class)"
--     • Modifier line: "[Shift+Ctrl]" (highlighted)
--     • Table rows: Button | Ability (+ cooldown) | Mana
--
-- Design goals:
--   • Keep UI logic isolated from hover filtering and binding resolution
--   • Measure text widths accurately (uses hidden FontStrings)
--   • Auto-size tooltip so ability names are not truncated
--   • Smart positioning near cursor while clamped on-screen
--   • Snappy fade in/out without visual jitter
--
-- Public API:
--   UI.Create(config) -> tipFrame
--   UI.Render(tip, config, headerText, modText, entries)
--
-- tipFrame methods provided by UI.Create:
...
-- -----------------------------------------------------------------------------
-- Creates and returns the tooltip frame + helper methods.
--
-- @param config table HoverCasts config
-- @return Frame tip
function UI.Create(config)
    local tip = CreateFrame("Frame", "HoverCastsTip", UIParent, "BackdropTemplate")
    tip:SetScale(config.scale or 1.0)
    ...
    -- Modifier line (just above table)
    ...
    -- Rows (Button | Action | Mana)
    tip.rows = {}
    local ROW_H = 12

    for i = 1, (config.maxLines or 12) do
    ...
    -- -------------------------------------------------------------------------
    -- Positioning: smart cursor anchoring + clamping
    -- -------------------------------------------------------------------------
...
-- Rendering + layout
-- -----------------------------------------------------------------------------
-- Renders rows and sizes the tooltip so columns fit without truncation.
--
-- @param tip Frame Tooltip frame created by UI.Create
-- @param config table Config
-- @param headerText string Already colorized
-- @param modText string Already wrapped/colored
-- @param entries table List of entries (caller sorts; caller may precompute actionText/manaText)
function UI.Render(tip, config, headerText, modText, entries)
    ...
    if not entries or #entries == 0 then
        ...
        shownCount = math.min(#entries, config.maxLines or 12)
        for i = 1, shownCount do
            ...
            row.mana:SetText(e.manaText or "")

            row:Show()
        end
    end

    -- Measure columns (unbounded width when available)
    ...
    -- Compute column positions and total width
    ...
    local totalW =
        pad + maxBtnW
        + (config.minActionGap or 4) + maxActionW
        + (config.manaGap or 8) + maxManaW
        + pad
